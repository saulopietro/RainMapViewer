...
import view.components.ColoredWaypoint;
 ...
 * @author alway
 */
public class AlertPageTest extends JFrame {
...
mapViewer.addMouseListener(new MouseAdapter() {
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        // Pega a cor para o nível selecionado
        Color fillColor = getColorForLevel(nivelField.getSelectedItem().toString());

        // Cria seu waypoint colorido customizado
        ColoredWaypoint wp = new ColoredWaypoint(selectedPos[0], fillColor);

        // Cria um Set com esse waypoint
        Set<ColoredWaypoint> waypoints = Set.of(wp);

        // Cria o painter parametrizado com ColoredWaypoint (mesmo tipo do Set)
        WaypointPainter<ColoredWaypoint> painter = new WaypointPainter<>();
        painter.setWaypoints(waypoints);

        // Define o renderizador para pintar o pin colorido
        painter.setRenderer((Graphics2D g, JXMapViewer map, ColoredWaypoint waypoint) -> {
            Point2D pos = map.convertGeoPositionToPoint(waypoint.getPosition());
            if (pos != null) {
                int x = (int) pos.getX();
                int y = (int) pos.getY();

                Color fillColor2 = waypoint.getColor();

                // Desenha um pin simples (triângulo + círculo)
                int pinWidth = 14;
                int pinHeight = 20;
                int[] xPoints = { x, x - pinWidth / 2, x + pinWidth / 2 };
                int[] yPoints = { y - pinHeight, y, y };
        ...
        // Aplica o painter no mapa e força repaint
...
private GeoPosition getCurrentLocationFromIP() {
    ...
    java.awt.EventQueue.invokeLater(() -> {
        new AlertPageTest().setVisible(true);
    });
}
